...
import { ARTIFACTS, NEEDS_ANALYSIS_WRITEUP_URL, SERIES_ACCENT, type ArtifactMeta } from './config';
...
export default function NeedsAnalysisHub(): JSX.Element {
  const showWriteup = NEEDS_ANALYSIS_WRITEUP_URL.trim().length > 0;
  ...
  return (
    <div className="mx-auto max-w-[820px] px-4 py-12 sm:px-8 lg:px-16 lg:py-14">
      ...
      {showWriteup && (
        <div className="mt-7">
          <a
            href={NEEDS_ANALYSIS_WRITEUP_URL}
            target="_blank"
          ...
          <p className="m-0 mt-2.5 font-sans text-caption text-tertiary" style={{ lineHeight: 1.5 }}>
            A longer piece on how the series came together.
          </p>
        </div>
      )}
